...
function CorporateGovernance({ id }) {
	const { t } = useTranslation()
	const isDesktop = useMediaQuery({ minWidth: 992 });

	return <div style={{ minHeight: '70vh', padding: (isDesktop ? '5rem 0 2rem 0' : '5rem 15px 2rem 15px'), borderEndStartRadius: 20 }} className={`bg-white`} id={id}>
		<Container>
			<Row className='mt-3'>
				<Fade className="mt-3" baseClassActive={'show'} >
					<Col className="ml-auto mr-auto" md="12" lg="15" sm="12">
						<h2 className="title mt-3 color-maxi-invert" style={{ fontWeight: '' }}>{t('sustentabilidade.governancia-corporativa.titulo')}</h2>
						{t('sustentabilidade.governancia-corporativa.texto', { returnObjects: true }).map(({ titulo, subtitulo, texto }, index) => <div key={index}>
