			...
			<Row className='mt-3'>
				<Fade className="mt-3" baseClassActive={'show'} >
					<Col className="ml-auto mr-auto" md="12" lg="15" sm="12">
						<h2 className="title mt-3 color-maxi-invert" style={{ fontWeight: '' }}>{t('sustentabilidade.financials.titulo')}</h2>
						{t('sustentabilidade.financials.texto', { returnObjects: true }).map(({ titulo, subtitulo, texto }, index) => <div key={index}>
